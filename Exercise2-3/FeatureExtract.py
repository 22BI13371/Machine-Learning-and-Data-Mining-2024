...
def RescaleImage(imageFile, NoOfScale):
    ...
    for i in range(5):
        # height_tmp= int (height /(2**i))
        # width_tmp = int (width *(2**i))
        img1 = cv2.resize(img, None, fx = 1/(2**i), fy = 1/(2**i))
    #     img1 = cv2.resize(img, (width_tmp,height_tmp))
...
# inputF = "Z:\My Drive\Research\iMorphSharedByHai\Datasets\LabeledData\LeftWings\egfr_F_R_oly_2X_1.tif"
